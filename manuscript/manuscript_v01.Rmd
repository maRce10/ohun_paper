---
title: 'ohun: an R package for diagnosing and optimizing automated sound event detection'
author:
output:
  html_document:
    toc: yes
...
```{r word count, eval = FALSE, echo = FALSE, results='asis'}
...
raven_detec <- raven_detec[, c("sound.files", "selec", "start", "end", "bottom.freq", "top.freq", "tuning_parameters")]
...
```{r, eval = FALSE, echo = TRUE}
diagnose_detection(reference = manual_ref_tae,
                   detection = raven_detec,
                   by = "tuning_parameters",
                   min.overlap = 0.5)
...
diagnose_detection(reference = manual_ref_tae, detection = raven_detec, by = "tuning_parameters", min.overlap = 0.5)

```

The output shows the indices described above. Plus three additional metrics specific to sound event detection: 'split positives', 'merge positives' and 'overlap to true positives'. 'Split positives' is the number of reduntant detections (i.e. those overlapping sounds already overlapped by other detections) by more than one detection, 'merge positives' the number of detected sounds overlapping with another detection and 'overlap to true positives' quantifies the mean overlap between detections and reference signal (1 means complete overlap). 


```{r, eval = FALSE, echo = TRUE}


diag_raven <- diagnose_detection(reference = manual_ref_tae, detection = raven_detec, by = "tuning_parameters")
...
The function also allows to detail those indices by sound file. Here we show the first 10 files:  
...
# opt_detec_train <- opt_detec_train[order(opt_detec_train$templates, opt_detec_train$f.score, decreasing = TRUE), ]

# sub_opt_detec_train <- opt_detec_train[ave(-opt_detec_train$f.score, opt_detec_train$templates, FUN = rank) <= 4, ]
  
sub_odt <- lapply(unique(opt_detec_train$templates), function(x) {
    
    X <- opt_detec_train[opt_detec_train$templates == x, ]
    X <- X[order(X$f.score, decreasing = TRUE), ]
    return(X[1:2, ])
    })
...
kbl <- kable(sub_opt_detec_train[, c("threshold", "templates", "true.positives", "false.positives", "false.negatives", "recall", "precision", "f.score")], row.names = FALSE, caption = "TABLE TEMPLATE PERFORMANCE. Performance diagnostic of template-based detections using four templates across several threshold values. Only the two highest performance iterations for each template are shown.", digits = 3)

kable_styling(kbl, bootstrap_options = c("striped", "hover",  full_width = FALSE))

```

We can explore the performance of each template in more detail by looking at the change in F1 score across thresholds (FIG. THRESHOLD vs F.SCORE).

```{r, eval = TRUE, echo = FALSE, fig.height= 3, fig.align='center', fig.cap= "FIG. THRESHOLD vs F.SCORE. Shows the changes in F1 score across the range of threshold values"}

agg_f1 <- aggregate(f.score ~ threshold + templates, data = opt_detec_train,
mean)

ggplot(agg_f1, aes(x = threshold, y = f.score, group = templates, color = templates)) +
    geom_line() + 
    geom_point() + 
...
# subset with highest performance
sub_opt_det_train <- opt_det_train[order(opt_det_train$f.score, decreasing = TRUE), ]

sub_opt_det_train <- sub_opt_det_train[1:10, c("threshold", "smooth", "hold.time", "min.duration", "max.duration", "true.positives", "false.positives",  "false.negatives", "recall", "precision", "f.score")]

head(sub_opt_det_train, 10)
```
...
```{r, diagnose 2, eval = TRUE, echo = TRUE}


diagnose_detection(reference = test_ref,
                   detection = det_test,
                   by.sound.file = FALSE,
                   macro.average = TRUE)
